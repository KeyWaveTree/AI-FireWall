...
from mitmproxy.tools.dump import DumpMaster #mitproxy의 DumpMaster 클래스를 통해 프록시 서비 시작

#차단 도메인 목록
BLOCKED_DOMAINS= ["example.com", "google.com"]
...
class DomainBlocker:
    ...
    def request(self, flow:http.HTTPFlow)->None:
        ...
        if host in BLOCKED_DOMAINS:#요청된 도메인이 차단 목록에 있으면
            flow.response = http.Request.make(403, #상태코드 403 (접근 금지)

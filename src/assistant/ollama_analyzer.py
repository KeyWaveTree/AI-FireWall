...
async def ollama_vulnerability_check(
    ...
    response_result = await AsyncClient().chat(
        model=model, messages=message, options={"temperature": 0.2, "top_p": 0.9}
    )
    response = response_result["response"]["content"]

    # 정규표현식 패턴 : 내가원하는 형태로 뽑아 log데이터 한테 보낼 수 있는 준비작업을 한다.
    ...
    log.threat = None
    log.threat_result = response  # 잘못된 응답도 일단을 기록
